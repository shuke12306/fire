...
- 用途：标签筛选话题列表
- 宿主备注：iOS / Android 首页 row tag 点击会把第一个标签作为 `TopicListQuery.tag` 传入共享 Rust 层，并与当前 Latest / New / Unread / Unseen / Hot / Top 筛选组合成 `/tag/{tag}/l/{filter}.json`；多标签会把剩余标签作为重复的 `tags[]`，并设置 `match_all_tags=true`。
- Query：
