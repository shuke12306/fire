...
P4 repository scaffolding 已完成，并已有 store media / internal testing / privacy review / performance / accessibility / release gate / roadmap plan contract / roadmap architecture constraints / roadmap implementation evidence / roadmap acceptance 校验脚本以及 `scripts/verify-release-readiness.sh` 总入口；最终验收仍依赖手动 store media、store records/test tracks/tester invites、maintainer/legal review、release-build 物理设备 benchmark，以及 VoiceOver/TalkBack/Dynamic Type/Reduce Motion/high-contrast audit 结果。

Store-media 校验要求最终素材文件名不能残留 fake/mock/placeholder 类标记，截图尺寸可解析且每边至少 320px，PNG 截图和 Play feature graphic 不能是单色占位图，截图目录只能包含常规图片文件，不能有嵌套目录或替代素材路径。App Preview 若存在必须是 `preview-video/app-preview.mp4` 常规文件并包含 MP4 `ftyp` 内容；Play feature graphic 必须是精确 `1024x500` PNG。

手动证据校验会拒绝 Complete/Accepted 行中残留 fake/mock/placeholder/dummy/synthetic/TODO/TBD/`example.com`/`not-real`/`not real` 标记。Evidence link 必须是 fully qualified HTTP(S) URL 或指向非空文件的安全 repo-relative path，且不能使用 dead local path、local directory、malformed URL、malformed URL host、single-label URL host、localhost/`.local`/`.test`/`.invalid` 这类占位 URL host。Date 必须是真实 `YYYY-MM-DD` 日历日期且不能是未来日期，表格行必须保持 exact Markdown column shape，并转义 cell 内的 literal `|`。

Performance result 必须包含与指标绑定的带单位数值；多内存值必须使用 `peak` 标注峰值，否则视为歧义结果，阈值比较使用最高的已标注 peak 值。Performance 每个平台/指标只能有一行，accessibility 每个平台/screen 或平台/audit category 只能有一行；manual release gate、internal testing、privacy review 证据行也不能重复。所有 `Accepted` waiver 必须在 notes 中包含明确 approval/waiver 语义和 reason/risk/exception/no-ship 说明，避免用伪造、占位或含糊证据关闭 P4。

`scripts/verify-p4-release-evidence-suite.sh` 提供 release readiness 和 checked P4 acceptance 共享的非递归 P4 证据套件；`scripts/test-release-verifiers.sh` 使用临时 fixtures 回归验证 release-readiness wrapper 与这些 release verifier 的 pass/fail 行为，包括 fake evidence、malformed store media、flat PNG placeholder、unexpected screenshot/preview entries、dead local evidence path、malformed evidence URL、malformed URL host、single-label URL host、placeholder URL host、invalid or future date、placeholder metadata field、missing/extra Markdown table column、non-measurement performance result、target misses marked `Pass`、ambiguous or misleading multi-value memory result、duplicate evidence rows 和 weak accepted-waiver notes，不替代真实人工证据。

`scripts/verify-roadmap-architecture-constraints.sh` 校验架构/平台边界、最低 API、iOS topic-detail native runtime path 和 reference/infrastructure 边界；`scripts/verify-roadmap-implementation-evidence.sh` 校验已勾选的 P1-P3 代码证据；`scripts/verify-roadmap-p4-acceptance.sh` 要求上方 P4 验收项保持精确命名，并在任何验收框被勾选时要求 shared P4 evidence suite 先通过。
