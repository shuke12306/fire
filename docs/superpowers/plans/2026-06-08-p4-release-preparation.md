...
7. **Make the final release gate fail closed.** `scripts/verify-release-gates.sh` checks the evidence register and fails until the exact required gate rows are present once, with no unknown rows, and every row has an accepted/completed status, owner, evidence link, and real calendar date. `Accepted` release-gate waivers must include explicit approval/waiver context plus a reason in notes, such as `Approved by ...; reason: ...`. Evidence links must be well-formed HTTP(S) URLs with hostnames or safe repo-relative paths to non-empty local files, and malformed URLs or placeholder URL hosts are rejected. Manual evidence verifiers reject completed or accepted rows that still contain fake/mock/placeholder-style markers in completion metadata such as owner, reviewer, tester, device, result, evidence link, or notes, and reject extra Markdown table columns from unescaped pipe characters. Current failure is expected because manual gates are still open.
