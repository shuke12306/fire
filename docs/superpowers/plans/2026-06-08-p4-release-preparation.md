...
7. **Make the final release gate fail closed.** `scripts/verify-release-gates.sh` checks the evidence register and fails until the exact required gate rows are present once, with no unknown rows, and every row has an accepted/completed status, owner, evidence link, and real calendar date. `Accepted` release-gate waivers must include explicit approval/waiver context plus a reason in notes, such as `Approved by ...; reason: ...`. Evidence links must be well-formed HTTP(S) URLs with valid hostnames or safe repo-relative paths to non-empty local files, not directories, and malformed URLs, malformed URL hosts, or placeholder URL hosts are rejected. Manual evidence verifiers reject completed or accepted rows that still contain fake/mock/placeholder-style markers in completion metadata such as owner, reviewer, tester, device, result, evidence link, or notes, and reject missing or extra Markdown table columns; literal pipe characters must be escaped in cells. Current failure is expected because manual gates are still open.
...
9. **Make benchmark evidence complete by metric and platform.** `scripts/verify-performance-benchmarks.sh` checks `performance-benchmarks.md` for iOS and Android release-build physical-device rows for every target metric, requires metric-bound numeric result units, rejects ambiguous multi-value memory results unless the peak value is labeled, treats the highest labeled peak memory value as authoritative, and fails until threshold failures are fixed or explicitly accepted with approval/waiver context plus a reason in notes.
...
- `scripts/test-release-verifiers.sh` is expected to pass without real manual evidence because it uses isolated temporary fixtures to regression-test the release verifiers themselves, including release-readiness wrapper wiring, fake-evidence marker rejection, malformed store-media failures, flat PNG placeholder rejection, non-measurement performance results, target misses marked `Pass`, ambiguous or misleading multi-value memory results, duplicate manual evidence rows, invalid calendar dates, placeholder metadata fields, dead local evidence paths, malformed evidence URLs, malformed URL hosts, placeholder URL hosts, extra Markdown table columns, missing Markdown table boundaries, weak accepted-waiver notes, and weak internal-testing/privacy metadata.
