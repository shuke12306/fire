...
7. **Make the final release gate fail closed.** `scripts/verify-release-gates.sh` checks the evidence register and fails until the exact required gate rows are present once, with no unknown rows, and every row has an accepted/completed status, owner, evidence link, and real calendar date. `Accepted` release-gate waivers must include explicit approval/waiver context plus a reason in notes, such as `Approved by ...; reason: ...`. Evidence links must be well-formed HTTP(S) URLs with fully qualified hostnames or safe repo-relative paths to non-empty local files, not directories, and malformed URLs, malformed URL hosts, single-label URL hosts, or placeholder URL hosts are rejected. Manual evidence verifiers reject completed or accepted rows that still contain fake/mock/placeholder-style markers in completion metadata such as owner, reviewer, tester, device, result, evidence link, or notes, and reject missing or extra Markdown table columns; literal pipe characters must be escaped in cells. Current failure is expected because manual gates are still open.
8. **Make store-media structure checkable before evidence closure.** `scripts/verify-marketing-assets.sh` validates that required screenshot buckets contain real PNG/JPEG files with readable dimensions, final asset filenames do not carry fake/mock/placeholder-style markers, screenshots are at least 320px on each side, PNG screenshots are not single-color placeholders, screenshot buckets do not contain nested directories or alternate asset paths, the optional App Preview path is unambiguous and contains MP4 `ftyp` content when present, and the Play feature graphic is PNG content with exact `1024x500` dimensions and non-flat pixels. The script is expected to fail until final release-candidate media exists.
9. **Make benchmark evidence complete by metric and platform.** `scripts/verify-performance-benchmarks.sh` checks `performance-benchmarks.md` for exactly one iOS row and exactly one Android release-build physical-device row for every target metric, requires metric-bound numeric result units, rejects ambiguous multi-value memory results unless the peak value is labeled, treats the highest labeled peak memory value as authoritative, and fails until threshold failures are fixed or explicitly accepted with approval/waiver context plus a reason in notes.
10. **Make accessibility evidence complete by screen and audit area.** `scripts/verify-accessibility-audit.sh` checks `accessibility-audit-checklist.md` for exactly one iOS row and exactly one Android physical-device row covering every listed screen and audit category, and fails until blocking failures are fixed or explicitly accepted with explicit approval/waiver context plus a reason in notes.
11. **Make testing-track evidence explicit.** `docs/release/internal-testing-evidence.md` records App Store Connect / Play Console records, release-candidate uploads, tester invites, and feedback triage. `scripts/verify-internal-testing-evidence.sh` fails until all required platform rows are complete and non-duplicated, requires evidence links to be fully qualified HTTP(S) URLs or existing local files, requires build numbers, commit SHAs, invite dates, and numeric blocker counts, and requires accepted rows to include explicit approval/waiver context plus a reason in notes.
12. **Make privacy review evidence explicit.** `docs/release/privacy-review-evidence.md` records maintainer/legal review of the privacy policy, store answers, backup behavior, diagnostic redaction, privacy manifests, license inventory, and final publication approval. `scripts/verify-privacy-review-evidence.sh` fails until all required review rows are complete and non-duplicated, requires evidence links to be fully qualified HTTP(S) URLs or existing local files, requires final publication approval wording, and requires accepted rows to include explicit approval/waiver context plus a reason in notes.
...
- `scripts/verify-marketing-assets.sh` is expected to fail while marketing folders contain only `.gitkeep` placeholders, while PNG assets are flat single-color placeholders, while final asset filenames still contain fake/mock/placeholder-style markers, while screenshot folders contain nested directories or alternate asset paths, or while screenshot/feature-graphic dimensions do not match the release constraints; it is a precondition for store-media evidence closure, not a replacement for human asset review.
...
- Manual evidence verifiers are expected to fail if completed or accepted evidence rows still contain fake, mock, placeholder, dummy, synthetic, TODO/TBD, `example.com`, `not-real`, or `not real` markers in owner, reviewer, tester, device, result, evidence-link, or notes metadata, if evidence links are dead local paths, local directories, malformed URLs, malformed URL hosts, single-label URL hosts, or placeholder URL hosts, if dates are impossible calendar dates or future dates, if rows have missing or extra Markdown table columns, if required manual evidence rows, platform/metric benchmark rows, or platform/screen and platform/audit accessibility rows are duplicated, if performance results lack metric-bound numeric units, leave multi-value memory results ambiguous, or hide a threshold miss behind a lower non-peak memory value, or if accepted waiver notes lack explicit approval/waiver context plus a reason, risk, exception, or no-ship decision.
...
- `scripts/test-release-verifiers.sh` is expected to pass without real manual evidence because it uses isolated temporary fixtures to regression-test the release verifiers themselves, including release-readiness wrapper wiring, fake-evidence marker rejection, malformed store-media failures, flat PNG placeholder rejection, non-measurement performance results, target misses marked `Pass`, ambiguous or misleading multi-value memory results, duplicate manual evidence rows, duplicate performance/accessibility screen or audit coverage rows, invalid or future dates, placeholder metadata fields, dead local evidence paths, malformed evidence URLs, malformed URL hosts, single-label URL hosts, placeholder URL hosts, extra Markdown table columns, missing Markdown table boundaries, weak accepted-waiver notes, and weak internal-testing/privacy metadata.
