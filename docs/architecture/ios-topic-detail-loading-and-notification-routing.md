...
Status update (2026-05-28): the first topic-detail paging performance hardening slice landed on both sides of the boundary. iOS now drives ListKit invalidation through a narrow `topicCollectionRevision`, applies initial screen loads and anchored hydration through coalesced detail/render transactions, and keeps anchor hydration out of the visible `loadingMore` footer state. The same slice also adds APM/signpost coverage around collection item-token resolution, diffable snapshot applies, and anchored hydration. Rust `fetch_topic_screen` now uses a lightweight `TopicDetail` parse that skips `thread` / `flat_posts` construction for the screen bootstrap path, caps response-page root branch loading to small fixed concurrency instead of page-wide fan-out, and logs duration for root-branch loads plus branch-index assembly.

Status update (2026-05-30): the active iOS topic-detail ListKit host now triggers `fetchTopicResponsePage` from rendered response-row tail proximity and collection scroll metrics, not from the maximum loaded `postNumber`. This matches Rust response-row preorder semantics where the largest floor number is not guaranteed to be the rendered last row. Pagination snapshots also use always-on scroll-anchor restoration, and Store-side response-page append filters pure overlapping rows so overlap does not force a full detail/render rebuild.

Status update (2026-05-30, feed-cache branch): topic-detail initial load now enters Rust through `loadTopicDetailFeed`, which wraps the existing `fetchTopicScreen` network path and persists the processed feed snapshot in `fire-store` SQLite storage under the app workspace cache. iOS bridges that snapshot back into the current `TopicScreenState` render path and renders it through the new `App/TopicDetailRuntime/` UIKit collection controller. That controller now uses IGListKit `ListAdapter`/section controllers for feed diffing and working-range lifecycle, Nuke 12.8.0 for image loading and prefetch, and Texture 3.2.0 through the local `TextureCore` XCFramework package for `ASTextNode`/`ASImageNode` row rendering. It also skips no-op IGListKit updates, only animates small attached-view updates while scrolling is idle, deduplicates scroll-driven visible-post notifications while force-publishing visibility after adapter updates, and keys row diff/layout identity off store-prepared render signatures instead of main-thread `cooked` HTML hashes. Incremental reply pagination remains on `fetchTopicResponsePage` until the Rust feed API grows a page-append command with the same cursor semantics.
...
- `native/ios-app/App/ListKit/TopicDetail/FireTopicDetailCollectionView.swift` `handleVisibleItemsChanged(_:)`, `handlePrefetchItems(_:)`, and `handleScrollMetricsChanged(_:)` plus `native/ios-app/App/FireTopicDetailView.swift` `handleVisiblePostNumbersChanged(_:)` -- visible-row and scroll-metric callbacks used to trigger response-page preload and reading-time timing.
