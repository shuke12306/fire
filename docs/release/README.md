...
content. Screenshot buckets must contain regular image files only, not nested
directories or alternate asset paths. If an App Preview is included,
`app-preview.mp4` must be a regular file with MP4 `ftyp` content; the preview
folder must not contain nested directories or alternate asset paths.
