...
Before marking P4 acceptance complete, run:

```bash
scripts/verify-release-gates.sh
```

The verifier is expected to fail while any evidence row is still `Not started`
or missing owner/link/date metadata.
