...
struct FireLDCView: View {
    ...
    @State private var authorization: LdcAuthorizationUrlState?
    ...
    var body: some View {
        List {
            ...
            if let userInfo {
                Section("信用指标") {
                    FireKeyValueRow(label: "累计收到", value: userInfo.totalReceive)
                    FireKeyValueRow(label: "累计支付", value: userInfo.totalPayment)
                    FireKeyValueRow(label: "累计转账", value: userInfo.totalTransfer)
                    FireKeyValueRow(label: "社区收入", value: userInfo.totalCommunity)
                    FireKeyValueRow(label: "剩余额度", value: userInfo.remainQuota)
                    FireKeyValueRow(label: "每日限额", value: "\(userInfo.dailyLimit)")
                }

                Section("账户权限") {
                    FireKeyValueRow(label: "支付评分", value: "\(userInfo.payScore)")
                    FireKeyValueRow(label: "支付等级", value: "\(userInfo.payLevel)")
                    FireKeyValueRow(label: "支付密钥", value: userInfo.isPayKey ? "已启用" : "未启用")
                    FireKeyValueRow(label: "管理员", value: userInfo.isAdmin ? "是" : "否")
                    if let gamificationScore = userInfo.gamificationScore {
                        FireKeyValueRow(label: "社区积分", value: "\(gamificationScore)")
                    }
                }
            }
    ...
    private func balanceOverview(_ info: LdcUserInfoState) -> some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack(alignment: .top, spacing: 12) {
                ...
                FireStatusChip(
                    label: info.isPayKey ? "支付可用" : "只读",
                    tone: info.isPayKey ? .success : .muted
                )
            }
            ...
            LazyVGrid(
                ...
                FireMetricTile(label: "可用余额", value: info.availableBalance)
                FireMetricTile(label: "社区余额", value: info.communityBalance)
            }
        }
        .padding(.vertical, 4)
    }
            ...
            if userInfo != nil {
                Button(role: .destructive) {
                    ...
                    HStack {
                        ...
                        Text(isLoggingOut ? "退出中…" : "退出 LDC 授权")
                            .fontWeight(.medium)
                        Spacer()
                    }
                }
    ...
    private func loadUserInfo(force: Bool) async {
        ...
        do {
            userInfo = try await viewModel.ldcUserInfo()
            errorMessage = nil
        } catch {
            if userInfo == nil {
    ...
    private func prepareAuthorization() async {
        ...
        do {
            let auth = try await viewModel.ldcAuthorizationUrl()
            authorization = auth
            approvalLink = try await viewModel.ldcApprovalLink(authorizationURL: auth.url)
            approvalStatus = nil
    ...
    private func completeAuthorization(approvalPath: String) async {
        ...
        do {
            let status = try await viewModel.ldcApprove(approvePath: approvalPath)
            ...
            case .approved:
                ...
                try await viewModel.ldcCallback(code: code, state: state)
                userInfo = try await viewModel.ldcUserInfo()
                noticeMessage = "LDC 授权已完成"
                errorMessage = nil
            case .pending:
                noticeMessage = "授权仍在等待确认"
                errorMessage = nil
            case .denied:
                errorMessage = "LDC 授权已被拒绝"
                noticeMessage = nil
            }
        } catch {
            errorMessage = error.localizedDescription
    ...
    private func logout() async {
        ...
        do {
            try await viewModel.ldcLogout()
            userInfo = nil
            authorization = nil
            approvalLink = nil
            approvalStatus = nil
            noticeMessage = "已退出 LDC 授权"
            errorMessage = nil
        } catch {
            errorMessage = error.localizedDescription
        }
