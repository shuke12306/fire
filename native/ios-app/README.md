...
- `App/Stores/FireTopicDetailStore.swift`
  ...
  - renders Rust-owned `TopicPostBoostState` short replies as a native Texture body overlay/barrage for original posts with visible body text, and as fixed chips for replies or posts without a body text target; the cell consumes Rust-derived `displayText` and keeps only fixed-chip height/signature in `FirePostCellLayoutKey` instead of parsing Boost `cooked` in Swift
