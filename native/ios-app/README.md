...
- Topic detail keeps a single reply-tree presentation path that shows every loaded Rust tree row with native Texture post cells. The toolbar does not expose an alternate display projection, and the controller runtime does not duplicate the Rust source/tree data path.
