...
- `App/FireTopicDetailView.swift`
  ...
  - now renders from store-prepared timeline rows and cached cooked text/image payloads so SwiftUI body updates do not repeatedly join posts to timeline entries, parse post HTML, or derive long cooked-content hashes
...
- `App/ListKit/`
  ...
  - topic detail now bypasses the generic diffable ListKit host and uses `App/TopicDetailRuntime/FireTopicDetailListViewController.swift`, where IGListKit owns the collection adapter, section controllers, working range, display callbacks, performance hooks, and update listener callbacks
...
- Swift Package dependencies are declared in `project.yml`: IGListKit 5.2.0, Nuke 12.8.0, CrashReporter, and ConfettiSwiftUI.
...
- Topic detail now renders its reading surface through a dedicated `TopicDetailRuntime` UIKit collection controller backed by IGListKit while keeping the SwiftUI shell for navigation, sheets, and the persistent quick-reply bar; it still hides the tab bar as a dedicated reading page and promotes the original post into the topic header as the main body. The controller skips no-op snapshot updates, suppresses list animations while the user is actively scrolling or while applying first/large updates, deduplicates scroll-driven visible-post reports, and force-publishes visibility after adapter updates complete. Header/category chips, AI summary, topic vote, original-post, and poll rows stay on SwiftUI-hosted cells so the IGListKit runtime preserves the established topic-detail visual treatment while native cells handle common reply rows.
- Topic detail initial load now enters Rust through `loadTopicDetailFeed`, which stores processed feed snapshots in the Rust SQLite `fire-store` cache before iOS bridges the snapshot back into the current `TopicScreenState` render path. Reply pagination still uses shared Rust `fetchTopicResponsePage`; the shared cursor is bounded by both root count and row count, so a single large reply branch continues through `nextBranchOffset` instead of blocking the iOS list on a huge one-page payload. The IGListKit runtime triggers the next response page from rendered-tail visibility and scroll-distance metrics instead of max `postNumber`, and filters pure overlap rows before applying incremental append snapshots.
- Topic-detail image loading is now Nuke-backed. `FireRemoteImagePipeline` owns a Nuke `ImagePipeline` with a 128 MB data cache, and `FireTopicImagePrefetchCoordinator` starts/stops Nuke prefetches from IGListKit working-range callbacks while preserving owner-local cancellation.
- Topic-detail cooked text and post images now render through Texture nodes inside native cells: `FirePostRichTextContainerView` uses `ASTextNode`, and post attachments use `ASImageNode` placeholders. Texture is consumed as a checked-in `AsyncDisplayKit.xcframework` linked directly by the app target, not through CocoaPods or a local SwiftPM package product.
...
- Topic detail cursor pagination now appends reply rows and cooked-content caches incrementally on a background task, coalesces detail/render/loading-footer updates into one list revision, races slow topic-detail operations against a real timeout, and keeps diff/layout tokens lightweight so large cooked HTML bodies are neither copied nor re-hashed during collection updates.
