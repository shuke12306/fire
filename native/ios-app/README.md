...
- `App/TopicDetail/`
  ...
  - `Support/FireTopicPresentation.swift` and the feed models/cells now render from store-prepared timeline rows and cached cooked text/image payloads so page updates do not repeatedly join posts to timeline entries, parse post HTML, or derive long cooked-content hashes
...
- Topic-detail cooked text and post images now render through Texture nodes inside native cells: `FirePostCellNode` uses `ASTextNode` for rich text, `ASNetworkImageNode` for avatars, and Nuke-backed `ASImageNode` post image nodes with inline RenderBlock ordering, loading/error retry states, display-size thumbnail preparation, and a UIKit zoom/pan image preview. Texture is consumed as a checked-in `AsyncDisplayKit.xcframework` linked directly by the app target, not through CocoaPods or a local SwiftPM package product.
...
- Topic detail now prepares reply rows plus cooked text/image payloads in the store layer, so runtime collection updates reuse cached render content instead of repeating timeline joins and cooked HTML parsing.
