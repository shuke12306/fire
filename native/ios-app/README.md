...
- Topic posts now render a richer native `RenderDocument` surface in the detail screen, including inline emoji attachments, linked-image extraction that prefers the original asset while preserving server-provided aspect ratios, Rust-filtered image metadata text such as `image 1080x1920 52.5kb`, compact quote/reply metadata blocks that ignore quote chrome/avatar images as post attachments, ordered/unordered lists, details/spoilers, simple tables, onebox/video placeholders, tappable `@mentions` / group mentions / hashtags, compact user sheets, in-app Safari for ordinary web links, and LinuxDo topic/profile/badge links that stay inside the app.
