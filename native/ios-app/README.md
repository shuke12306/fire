...
- Topic detail now supports per-post reply targeting, quote reply insertion from Rust-provided `RenderDocumentState.plainText`, post likes, a searchable Rust-bootstrap-backed reaction picker with reaction-user lookup, post-level bookmarks with native reminder scheduling and local notifications, edit/delete/recover, and flag reporting through the shared Rust write APIs; the flag sheet uses server-provided post action types when available and falls back to the common LinuxDo / Discourse flag types only if that metadata is unavailable.
- Topic detail now shows clickable reply targets using shared `replyToUser` metadata when available, and collapsed inline reply branches can expand the already loaded child replies in place. The native reply-context sheet remains available for reply-context loading flows that use Rust-backed recursive reply IDs, batched `fetchTopicPosts`, reply history, and floor jumps.
- Topic detail now has a toolbar view-mode switch between the existing collapsed conversation projection and a threaded projection that shows every loaded Rust tree row with native Texture post cells. The switch only changes the controller-local runtime snapshot projection; it does not reintroduce SwiftUI post rows or duplicate the Rust source/tree data path.
