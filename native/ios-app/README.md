...
- The app now also exposes a full-screen native composer for create-topic, private-message, and advanced-reply flows, with server-backed draft restore/save, recipient search, image uploads, upload URL resolution, tag search, selection-aware Markdown formatting, and `@mention` autocomplete on top of shared Rust APIs.
